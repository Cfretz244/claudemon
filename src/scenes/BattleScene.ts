import { executeBattleMove, type Combatant, trainerPrizeMoney } from '@claudemon/engine';
...
import { resolveTurnOrder, applyLeechSeed, calculateRunChance, splitExp, getFirstAlivePokemon } from '../systems/BattleEngine';
...
import { playMoveAnimation } from '../systems/MoveAnimations';
...
export class BattleScene extends Phaser.Scene {
  ...
  private async executeMove(
    ...
    const player: Combatant = { pokemon: this.playerPokemon, stages: this.playerStatStages,
      volatile: this.playerVolatile, disable: this.playerDisable, recharging: this.playerRecharging };
    const opponent: Combatant = { pokemon: this.opponentPokemon, stages: this.opponentStatStages,
      volatile: this.opponentVolatile, disable: this.opponentDisable, recharging: this.opponentRecharging };
    const event = executeBattleMove(isPlayer ? player : opponent, isPlayer ? opponent : player, attacker.moves.indexOf(move));
    this.playerRecharging = player.recharging;
    this.opponentRecharging = opponent.recharging;
    if (event.chargeCancelled) this.clearCharge(isPlayer);
    if (event.before.length) await this.showText(event.before);
    if (event.moveId) {
      const sprite = isPlayer ? this.playerSprite : this.opponentSprite;
      await playMoveAnimation(event.moveId, { scene: this, attackerSprite: sprite,
        defenderSprite: isPlayer ? this.opponentSprite : this.playerSprite, isPlayer,
        phase: event.phase, outcome: event.outcome });
      if (event.phase === 'release') sprite.setAlpha(1);
      if (event.hit) soundSystem.hit();
    }
    await Promise.all([
      this.hud.animatePlayerHP(this.playerPokemon.currentHp / this.playerPokemon.stats.hp),
      this.hud.animateOpponentHP(this.opponentPokemon.currentHp / this.opponentPokemon.stats.hp),
    ]);
    this.updateHUD();
    if (event.messages.length) await this.showText(event.messages);
  }
