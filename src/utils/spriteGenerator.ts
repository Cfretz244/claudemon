...
/**
 * Overworld sprite for a static legendary standing in the world (the three
 * birds, Mewtwo). Draws the species' 32px battle sprite once and reuses that
 * one pose for all 4 directions x 2 walk frames, so the legendary looks big
 * and never animates.
 */
export function generateLegendaryNPCSprite(scene: Phaser.Scene, key: string, speciesId: number): void {
  const SIZE = 32; // Full Pokemon sprite size so the legendary looks big
  const canvas = document.createElement('canvas');
  canvas.width = SIZE * 4;
  canvas.height = SIZE * 2;
  const ctx = canvas.getContext('2d')!;

  // Render the Pokemon sprite once, reuse for all frames
  const pokemonCanvas = document.createElement('canvas');
  pokemonCanvas.width = SIZE;
  pokemonCanvas.height = SIZE;
  const pokemonCtx = pokemonCanvas.getContext('2d')!;
